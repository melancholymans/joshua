	...
	#include <vector>   //generate_evasions TESTのため
	#include <algorithm>
#endif
#include "movegen.h"
...
void MovePicker::go_next_phase()
{
	...
	case PhTacticalMove0: case PhTacticalMove1:
		...
		return;
	case PhKiller:
		//TODO:killerの準備ができていないのでreturnするだけ
		//killerの準備ができていないのでm_phaseを１つ進める
		//killerの準備ができたら削除すること
		m_phase++;
		//return;
...
TEST(movepicker, movepicker_POS37_black)
{
	...
	for (int i = 0; i < 10; i++){
		m = mp.next_move();
		if (old_m.end() != std::find(old_m.begin(), old_m.end(), m)){	//next_move関数が同じ手を返していないことのcheck
			EXPECT_TRUE(false);
		}
		EXPECT_TRUE(array_check(m, ml));
		old_m.push_back(m);
